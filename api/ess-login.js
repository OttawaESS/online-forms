      ...
      <div class="container-fluid p-5">
        <div class="row justify-content-center">
          <div class="card shadow-sm">
            <div class="card-body">
              <h4 class="mb-3 text-center">ESS Officer Login</h4>
              <form method="POST">
...
export default async function handler(req, res) {
  ...
  if (hash === ESS_PASSWORD_HASH) {
    const secret = process.env.ESS_SESSION_SECRET;
    if (!secret) {
      res.statusCode = 500;
      return res.end('Missing ESS_SESSION_SECRET');
    }
    ...
    setCookie(res, 'ess_token', token, { maxAge: 60 * 60 * 24 });
    res.statusCode = 302;
    res.setHeader('Location', '/ess');
    return res.end();
  }

  res.statusCode = 302;
  res.setHeader('Location', '/ess-login?error=1');
  return res.end();
}